...
## 文档指针
- **改动历史（changelog，按日期）**：`workspaces/docs/plans/2026-*-danqingyuan-v2-changelog.md`（06-11起，最新 **07-05 第七日整体重构**[考后日常+日终收尾序列+秘阁前置隐藏+靖康强化，prompt scene v23/dialogue v11]；07-02 秘阁五幕重做+8线索；06-27 含成长数值+温书自测+结局序列+世界观重构）。
...
- **下一个主攻**：场景图池接入轮（美术第二批 P0 53张 `docs/plans/2026-07-03-danqingyuan-v2-art-assets-p0.md` 到位后：场景图池选择器[地点×时段×活动/speaker，用上院堂 class/afterclass/rainday 等变体]+半身像 char/→bust/ emotionState 差分+各页背景替换）→ 扩充择端/嵩/李唐好感线（各埋骸游图分工线索）→ 秘阁出现的文字引导+美术加强（明明说突兀，后补）。第七日重构 Stage1~3 已完成（考后日常+日终收尾序列+秘阁隐藏+练习封顶反馈/体力平衡/希孟首遇清干扰/练习签模板化/院堂换图）。
