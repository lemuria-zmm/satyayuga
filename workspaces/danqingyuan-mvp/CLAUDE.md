...
- **下一个主攻**：第七日重构后续两阶段（Stage2 P1：后花园练习签数值显示查修 + 数值平衡[收益高则消耗低，待明明指认具体卡] + 希孟初遇规则[取消书房前不出场规则/严格写初遇循序渐进，改 scene prompt]；Stage3 P2：固定成长签改模板句省 token + 食物/娱乐文化小百科 Tips + 主界面换 bg-the-main-hall）→ 美术第二批产出（VN化配套 P0 53张，清单 `docs/plans/2026-07-03-danqingyuan-v2-art-assets-p0.md`）+ 到位后工程接入轮 → 扩充择端/嵩/李唐好感线（各埋骸游图分工线索）。
