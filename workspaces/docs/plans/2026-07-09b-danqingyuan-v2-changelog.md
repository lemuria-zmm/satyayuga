...
## 十、再追加（明明看图反馈五，同日）
1. **"随小书童入院"后黑屏**：AdmissionTransition 预载入院后首屏（`bg-main-hall-morning` + `dlg/dialogue-scroll-bg`），黑闪消除。
2. **入院名录右侧竹签文字超框**：文案缩短「入院七日修习丹青，月末应丹青试」+ 字号 13→11.5、行高 1.6→1.4、内框加高，单列容下不换列。
3. **入院引导嵩全身像弹两次**：本科句(行7)与导师介绍句(行5)对同一导师重复弹立绘——删 `lineImages[7]`（本科句不再弹，导师只在各自介绍句弹一次）。
