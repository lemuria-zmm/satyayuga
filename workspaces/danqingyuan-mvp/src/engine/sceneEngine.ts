...
export function buildSceneFacts(prevState: GameState, action: GameAction, locationId: LocationId): string[] {
  ...
  if (action.type === 'wander') {
    // 希孟画室（好感≥知己解锁）：专属场景，希孟引玩家入画室、示画、吐露心迹（2026-06-25；2026-06-30 重构）
    if (locationId === 'ximeng_studio') {
      facts.push('玩家来到希孟画室——这是交情到了知己才得入的地方');
      facts.push('案上摊着他那卷要进献的《千里江山卷》（青绿山水，盛世气象）；而墙根另有一卷半掩着，画意与之截然不同，他并不主动提起');
      facts.push('此处只宜两人静处论画、吐露心迹，不必再客套；希孟或会松口一两句平日不说的话，但不点破到底');
      return facts;
    }
    // 书房线索 3（2026-06-30，克制）：好感≥同道(40) 进书房信步时，撞见希孟案上另一卷匆匆收起
    if (locationId === 'library' && (prevState.relationships?.ximeng?.hiddenAffinity ?? 0) >= 40) {
      facts.push('书房一角，希孟正伏案——你瞥见他手边除了那卷常画的青绿，还压着另一卷；见你来，他不动声色地将那卷掩了过去');
    }
