...
- ✅ **结局序列重设计批一（06-30）**：丹青试交卷后改多段立绘演出——【导师点评(LLM)】→【授衔(CSS朱印授祗候+七日回顾+解锁入口)】→【收尾动画(打字机)】。落第补考(retake)/见希孟(C/D)**先留桩批二补**。新建 `engine/endingSequence.ts`（EndingStage 纯函数状态机）+ EndingDialogue/TitleGrantOverlay/EpilogueScreen 组件；rank/解锁推迟到授衔段提交（落第补考保底过才授名分）；character_dialogue v6 加 examReview 点评指引（前后端已 v6 一致）。
- **下一个主攻**：结局序列批二（补考完整 ExamScreen retake 保底过 + 引希孟线 C 过场 + 见希孟 D）→ 靖康暗线（结局引出历史前奏，半架空）+ 小测以诗入画（改 prompt）→ 秘阁三幕重做+8张线索 → 扩充择端/嵩/李唐好感线。
