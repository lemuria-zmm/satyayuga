...
- ✅ **丹青试目标线（06-28）**：技能 gating（本科<40 封顶59分）+ 多维结局（分数定档优画待诏/良画正解锁秘阁/中勉过/落第，好感+暗线修饰）+ 独立结局页 EndingScreen（七日养成回顾，固定模板不调LLM）。
- **下一个主攻**：秘阁三幕重做+8张线索 → 扩充择端/嵩/李唐好感线 → 地图引擎（待出图）。
- **待补美术/增强**：温书自测 UI + 结局页各档卷轴/配图（与结局 LLM 散文增强一起后补，见 project-memory 待补美术区）。
