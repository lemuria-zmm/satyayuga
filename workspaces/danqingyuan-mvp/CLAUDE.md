...
- **下一个主攻**：场景图池接入轮（美术第二批 P0 53张 `docs/plans/2026-07-03-danqingyuan-v2-art-assets-p0.md` 到位后：场景图池选择器[地点×时段×活动/speaker，用上院堂 class/afterclass/rainday 等变体]+半身像 char/→bust/ emotionState 差分+各页背景替换）→ 扩充择端/嵩/李唐好感线（各埋骸游图分工线索）→ 秘阁出现的文字引导+美术加强（明明说突兀，后补）。第七日重构 Stage1~3 + 天气系统 + 丹青试改版（1选项题+1自由创作，画案手记择灵感→LLM命题→写创作思路→加权评档）已完成。
